...
const OrderPlaceScreen: React.FC<Props> = ({ route }) => {
    ...
    const increaseQty = async (item: SurpriseBoxData, index: number) => {
        let didUpdate = false;

        const loadingData = surpriseBoxData.map((i, ind) =>
            ind === index
                ? { ...i, incLoading: true }
                : { ...i, incLoading: false }
        );

        dispatch(setMealDataList(loadingData));
        setSurpriseBoxData(loadingData);

        const updatedData = loadingData.map((i, ind) => {
            if (ind === index && i.quantity < i.noOfBoxRemaing) {
                didUpdate = true;
        ...
        if (!didUpdate) return;
        const finalData = updatedData.map(i => ({
            ...i,
            decLoading: false,
        }));

        dispatch(setMealDataList(finalData));
        setSurpriseBoxData(finalData);
    };

    const decreaseQty = async (item: SurpriseBoxData, index: number) => {
        let didUpdate = false;
                                        ...
                                        }
                                        {
                                            (showRemoveCoupon) && <View style={{ marginTop: 5, marginBottom: 0 }}>
                                                <View style={{ display: "flex", flexDirection: "row", alignItems: "center" }}>
                                                    ...
                                                    <Text style={[styles.applyText, { color: "#2E2E2E", marginLeft: 4 }]}>You will get {couponPercentage} with</Text>
