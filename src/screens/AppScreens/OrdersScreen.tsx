...
import { setSurpriseBoxImage } from '../../store/Slices/mealQuantitySlice';
...
const OrdersScreen: React.FC = () => {
  ...
  const route =
    useRoute<RouteProp<PickStackParamList, 'PickupConfirmationScreen'>>();
  ...
  const getOrderListByUserIdFunction = async () => {
    const toekn = await AsyncStorage.getItem('userData');
    const user_id = await AsyncStorage.getItem('user_id');
    let orderStatus;
        ...
        >
          ...
          <View style={styles.contentContainer}>
            ...
            <View style={[styles.scrollContent, { paddingBottom: 10 }]}>
              ...
              >
                {['Active Orders', 'Completed', 'Cancel']?.map(
                  (item, index) => {
                    return (
                      <Pressable
                        ...
                        style={{
                          backgroundColor:
                            item == selectedItem ? '#264941' : 'transparent',
                          paddingVertical: 8,
                      ...
                      >
                        <Text
                          style={{
                            color: item == selectedItem ? '#fff' : '#264941',
            ...
            {loader ? (
                        ...
                        >
                          ...
                          >
                            <Text style={styles.orderNumber}>
                              Order #: {item_data?.orderID}
                            </Text>
                            <Text
                              style={[
                                styles.orderNumber,
                                { color: '#FF9D00', fontSize: fontPixel(16) },
                              ]}
                            >
                              {item_data?.groupedItems
                        ...
                        >
                          <View style={{ flex: 1 }}>
                            ...
                            >
                              ...
                              >
                                Order #:{' '}
                                {item?.orderID?.length > 15
                                  ? item?.orderID?.slice(0, 15) + '...'
                                  : item?.orderID}
                              </Text>
              ...
              >
                ...
                >
                  ...
                  >
                    {selectedItem === 'Active Orders' &&
                      "You're all caught up! No active orders right now. Place a new order to get started."}
                    {selectedItem === 'Completed' &&
                      'Nothing here yet. You don’t have any completed orders right now.'}
                    {selectedItem === 'Cancel' &&
                      'No cancelled orders found. All your orders are going smoothly.'}
                    {/* {"No Data Found"} */}
                  </Text>
                </View>
              </View>
