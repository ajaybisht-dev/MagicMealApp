...
import {
  setAddressData,
  setRadiusData,
} from '../../store/Slices/locationRadiusSlice';
import { RootState } from '../../store/rootReducer';
import { getData } from '../../utils/storage';
import AllOffersComponent from '../../component/AllOffersComponent';
import {
  getAllActiveSurpriseBoxesByCustomerId,
  getAllHotDealSurpriseBoxes,
...
const HomeScreen: React.FC = () => {
  ...
  const vendor_type_Selector = useSelector(
    (state: RootState) => state?.ventorAndMealSlice?.vendor_types,
  );
  const meal_type_Selector = useSelector(
    (state: RootState) => state?.ventorAndMealSlice?.meals_types,
  );
  ...
  const [defaultSelectedVendorsIds, setDefaultSelectedVendorsIds] = useState<
    string[]
  >([]);
  const [selectedMealsIds, setSelectedMealsIds] = useState<string[]>([]);
  const [defaultSelectedMealsIds, setDefaultSelectedMealsIds] = useState<
    string[]
  >([]);
  ...
  const [userTotalSaving, setUserTotalSaving] =
    useState<UserTotalSaving | null>(null);
  ...
  useEffect(() => {
    ...
    const unsubscribe = parent.addListener('tabPress' as never, (e: any) => {
  ...
  useEffect(() => {
    ...
    if (meal_type_Selector) {
      ...
      let isDispalyData = meal_type_Selector?.filter(
  ...
  const toggleSelection = async (
    ...
    if (item !== 'Restaurant') {
      ...
      const selectedMealsIdsData = selectedMealsIds.splice(
        0,
        selectedMealsIds?.length,
      );
    }
  ...
  async function getAllActiveSurpriseBoxesByCustomerIdFunction(
    priceLimit: any,
  ) {
    ...
    let payload = {
      ...
      pageSize: 10,
      vendorTypeIDs:
        selectedVendorsIds.length > 0
          ? selectedVendorsIds
          : defaultSelectedVendorsIds,
      mealTypeIDs:
        selectedMealsIds.length > 0
          ? selectedMealsIds
          : defaultSelectedMealsIds,
      maxDistanceKm: Number(radiusSelector),
      minPrice: 0,
      maxPrice: priceLimit * 10,
    ...
    await getAllActiveSurpriseBoxesByCustomerId(payload).then(response => {
      if (response?.succeeded) {
        ...
        setOffers(prev => {
          if (pageNumber.current === 1) return response?.data;
          const combined = [...prev, ...response?.data];
          return combined.filter(
            (item, index, self) =>
              index ===
              self.findIndex(t => t.surpriseBoxID === item.surpriseBoxID),
          );
        });
  ...
  const handleImageType = (mealType: any) => {
    switch (mealType) {
      case 'Non Veg':
        return Icons.NonVegIcon;
      case 'Vegan':
        return Icons.VeganIcon;
      case 'Sea Food':
        return Icons.SeaFoodIcon;
      case 'Veg':
        return Icons.VegIcon;
      case 'Others':
        return Icons.VegBoxIcon;
      default:
        return Icons.VegIcon;
      ...
      <View style={styles.categoryPage}>
        {item.map((cat: any) => (
          <Pressable
            ...
            style={[
              styles.categoryCard,
              { backgroundColor: handleBackGroundColor(cat) },
            ]}
            onPress={() =>
              toggleSelectionCategory(
                cat?.mealTypeName,
                cat?.id,
                selectedMeals,
                setSelectedMeals,
          ...
          >
            <View
              style={{
                backgroundColor: handleImageBackgroundType(cat.mealTypeName),
            ...
            >
              <Image
                source={handleImageType(cat.mealTypeName)}
                style={styles.categoryImage}
                resizeMode="contain"
                tintColor={handleTintColor(cat.mealTypeName)}
              />
            </View>
            <Text
              style={[styles.categoryName, { color: handleMealCategory(cat) }]}
            >
              {cat.mealTypeName}
            </Text>
          </Pressable>
      ...
      <View style={styles.dealPage}>
        {item.map((deal: any, index: any) => (
          <Pressable
            ...
            onPress={() => handleNavigation(deal)}
          >
            <View style={styles.dealHeader}>
              <View style={{ height: 'auto' }}>
                <Text style={styles.dealRestaurant} numberOfLines={2}>
                  {deal.serviceProviderName}
                </Text>
              </View>
              {deal?.spRating && (
                <View
                ...
                >
                  <StarRatingComponent
                    ...
                    showText={false}
                  />
                  <Text style={styles.dealRating}>
                    {' '}
                    {deal.spReviewCount
                      ? deal.spReviewCount > 1
                        ? deal.spReviewCount + ' Reviews'
                        : deal.spReviewCount + ' Review'
                      : ''}
                  </Text>
                </View>
              )}
            ...
            <View style={{ justifyContent: 'center', alignItems: 'center' }}>
              <Image
                source={handleImageData(
                  deal?.sbImageURL,
                  deal?.surpriseBoxMealType[0]?.mealType,
                  vendorTypeData,
                  deal?.serviceProviderVendorType,
                )}
              ...
              <ImageBackground
                ...
                resizeMode="contain"
              >
                <Text style={[styles.discountText, { textAlign: 'center' }]}>
                  {`${deal.discountedPercent}`}
                  <Text style={{ fontSize: fontPixel(10) }}>%</Text>
                </Text>
                <Text style={[styles.discountText, { textAlign: 'center' }]}>
                  OFF
                </Text>
            ...
            <View style={{ alignItems: 'center' }}>
              ...
              <Text style={styles.dealPrice}>
                <Text style={styles.oldPrice}>AED {deal.actualPrice}</Text> AED{' '}
            ...
            >
              ...
              <Text style={styles.dealInfo}>
                {' '}
                {`${deal.collectionFromTime}`}
              </Text>
              {deal?.timeFromAMPM != deal?.timeToAMPM ? (
                <Text style={styles.dealInfo}>
                  {' '}
                  {`${deal?.timeFromAMPM}`} -
                </Text>
              ) : (
                <Text style={styles.dealInfo}> -</Text>
              )}
              <Text style={styles.dealInfo}>
                {' '}
                {`${deal.collectionToTime}`} {deal.timeToAMPM}
              </Text>
            </View>
  ...
  const handleSearchText = async (text: string) => {
    ...
    if (text.length >= 3 || text.length === 0) {
      await getAllActiveSurpriseBoxesByCustomerIdFunction(50);
    }
  };
  ...
  const handleClearSearchText = async () => {
    ...
    setSearchText('');
    pageNumber.current == 1;
    const emptyArray = offers.splice(0, offers?.length);
    setOffers([]);
    setSelectedMeals([]);
    setSelectedMealsIds([]);
    setSelectedVendors([]);
        ...
        >
          <View style={{ paddingLeft: 10, paddingRight: 10 }}>
            ...
            <View style={styles.searchContainer}>
              ...
              />
              {searchText && (
                <Pressable onPress={() => handleClearSearchText()}>
                  <Image
          ...
          >
            ...
            <View
              style={{
                marginBottom:
                  userTotalSaving && userTotalSaving?.savingPrice > 0 ? 0 : 20,
              }}
            >
              {userTotalSaving && userTotalSaving?.savingPrice > 0 && (
                <TotalSaving
                  userTotalSaving={userTotalSaving}
                  accountDate={null}
                />
              )}
            </View>
                ...
                {!searchText && (
                  <View>
                      ...
                      {vendorsType?.map((vendor: any, index) => (
                        <Pressable
                          ...
                          style={{
                            ...
                            alignItems: 'center',
                            justifyContent: 'center',
                            backgroundColor: selectedVendors.includes(
                              vendor?.vendorTypeName,
                            )
                              ? '#264941'
                              : 'transparent',
                        ...
                        >
                          <Text
                            style={{
                              ...
                              color: selectedVendors.includes(
                                vendor?.vendorTypeName,
                              )
                                ? '#ffffff'
                                : '#264941',
                    ...
                    {selectedVendors.includes('Restaurant') && (
                      <View>
                        <View>
                          <Text style={styles.hotDealsTitle}>
                            {HomeLabels.BrowseByCategory}
                          </Text>
                        </View>
                    ...
                    {!searchText && (
                      <View style={styles.hotDealsSection}>
                        <View style={styles.sectionTitleContainer}>
                          <Text style={styles.hotDealsTitle}>
                            {HomeLabels?.HotDealsNearYou}
                          </Text>
                          <View style={styles.dotsContainer}>
                            {groupedDeals.slice(0, 3).map((_, index) => (
                              <View
                                key={index}
                                style={[
                                  styles.dot,
                      ...
                      <View style={styles.allOffersHeader}>
                        {searchText ? (
                          <View style={styles.allOffersLeft}>
                            <Text style={styles.allOffersTitle}>Results</Text>
                            <Text style={styles.allOffersTitle1}>
                              {' '}
                              {offers?.length > 1
                          ...
                          <View style={styles.allOffersLeft}>
                            <Text style={styles.allOffersTitle}>
                              {selectedVendorsIds?.length > 0 ||
                              selectedMealsIds?.length > 0
                                ? HomeLabels?.Offers
                                : HomeLabels?.AllOffers}
                            </Text>
                            {(selectedVendorsIds?.length > 0 ||
                              selectedMealsIds?.length > 0) && (
                              <Text style={styles.allOffersTitle1}>
                  ...
                  >
                    ...
                    <Pressable
                      ...
                      style={{
                        ...
                        paddingVertical: 10,
                        marginTop: '5%',
                        borderWidth: noDataMessage3 ? 1 : 0,
                        borderColor: '#264941',
                      }}
                    >
                      <Text
