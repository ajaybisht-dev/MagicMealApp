...
type CheckoutNavigationProp = DrawerNavigationProp<DrawerParamList, "OrderConfirmationScreen">;
// type Props = DrawerScreenProps<DrawerParamList, "OrderConfirmationScreen">;
type Props = NativeStackScreenProps<PickStackParamList, "OrderCompleted">;

const OrderCompleted: React.FC<Props> = ({ route }) => {
    ...
    const vendorImageSelector = useSelector((state: RootState) => state?.mealQuantitySlice?.surpriseBoxIdImage);
    ...
    const getOrdersConfirmDetailsByOrderID = async () => {
        ...
        await getOrderByOrderIdApi(payload).then((response) => {
            setLoader(false);
            if (response?.succeeded) {
                for (const item of response?.data?.items) {
                    const surpriseBox = vendorImageSelector.find(
    ...
    //             case "Others": return Icons.BakeryIcon
    //             default:
    //                 break;
    //         }
    //     } else {
    ...
    const handlePriceCalculation = (key: string) => {
        if (!Array.isArray(orderConfirmationDetails?.items) || orderConfirmationDetails?.items.length === 0) {
            return "AED 0.00";
        }

        const actualTotal = orderConfirmationDetails?.items.reduce(
            (sum: any, item: any) => sum + (item.actualPrice || 0) * (item.quantity || 0),
            0
        );

        const discountedTotal = orderConfirmationDetails?.items.reduce(
            (sum: any, item: any) => sum + (item.discountedPrice || 0) * (item.quantity || 0),
            0
        );

        switch (key) {
            case "offer": {
                const savedAmount = (actualTotal - discountedTotal) + orderConfirmationDetails?.discountAmount;
                ...
                <ImageBackground source={Images.LogoBg} style={{ height: heightPixel(350), flex: 1 }} resizeMode="contain" imageStyle={{ tintColor: "#23302A" }}>
                    <View style={styles.headerNav}>
                        <Pressable style={styles.header} onPress={() => navigation.goBack()}>
                            <Image source={Icons.BackIcon} resizeMode="contain" style={{ height: heightPixel(20), width: widthPixel(20), marginRight: 10 }} />
                                ...
                                <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
                                    <View style={styles.successSection}>
                                        <View style={{ marginTop: 10, alignItems: "center" }}>
                                            <Text style={styles.transactionText}>Transaction ID: {orderConfirmationDetails?.transactionID}</Text>
                                        </View>
                                    </View>

                                    <View style={{ display: "flex", flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between" }}>
                                        <Text style={styles.orderNumber}>Order #: {orderConfirmationDetails?.orderID}</Text>
                                        <View style={{ display: "flex", flexDirection: "row", alignItems: "flex-start" }}>
                                            <Image source={Icons.InvoiceIcon} resizeMode="contain" style={{ height: heightPixel(25), width: widthPixel(25) }} />
                                            <Text style={[styles.actionText, { color: "#109D7D" }]}>Invoice</Text>
                                        </View>
                                    </View>
                                    <View>
                                        {
                                            Object.entries(orderConfirmationDetails?.groupedItems || {}).map(
                                                (entry) => {
                                                    ...
                                                    return (
                                                        <View key={providerName} style={styles.card}>
                                                            {items.map((item: any, index: number) => {
                                                                return (
                                                                    <Pressable key={index}
                                                                        onPress={() => navigation.navigate("PickupConfirmationScreen", {
                                                                            orderId: orderConfirmationDetails?.id,
                                                                            totalAmount: item?.unitPrice * item?.quantity,
                                                                            serviceProviderId: item?.serviceProviderId,
                                                                            orderNumber: orderConfirmationDetails?.orderID,
                                                                            savedAmount: savedAmountRef.current,
                                                                            transactionID: orderConfirmationDetails?.transactionID,
                                                                            routeId: 1,
                                                                        })}
                                                                    >
                                                                        {
                                                                            ...
                                                                            <View style={styles.cardHeader}>
                                                                                ...
                                                                                <View style={{ flexDirection: "row", alignItems: "center" }}>
                                                                                    <Image source={Icons.ArrowGreenIcon} resizeMode="contain" style={{ height: heightPixel(15), width: widthPixel(10) }} tintColor={"#9D9D9D"} />
                                                                                    <Image source={Icons.ArrowGreenIcon} resizeMode="contain" style={{ height: heightPixel(15), width: widthPixel(10) }} tintColor={"#9D9D9D"} />
                                                                                </View>
                                                                            </View>
                                                                        }
                                                                        {/* Item Row */}
                                                                        <View style={[styles.itemRow, index < items.length - 1 && { borderBottomWidth: 1, marginBottom: 8, borderBottomColor: "#DDDDDD" }]}>
                                                                            <View style={styles.boxWhiteInner}>
                                                                                {/* <Image source={handleImageData(item?.sbImageURL, item?.surpriseBoxMealType[0]?.mealType)} resizeMode="contain" style={{ height: heightPixel(72), width: widthPixel(72) }} /> */}
                                                                                <Image source={handleImageData(item?.sbImageURL, item?.surpriseBoxMealType[0]?.mealType, item?.serviceProviderVendorType)} resizeMode="contain" style={{ height: heightPixel(72), width: widthPixel(72) }} />
                                                                            ...
                                                                            <View style={styles.itemDetails}>
                                                                                ...
                                                                                <View style={styles.priceRow}>
                                                                                    <Text style={styles.oldPrice}>AED {item?.actualPrice * item?.quantity}</Text>
                                                                                    <Text style={styles.newPrice}>AED {item?.discountedPrice * item?.quantity}</Text>
                                                                                    <Text style={styles.discountText}>{item?.discountedPercent}% OFF</Text>
                                                                                </View>
                                                                        ...
                                                                        {index === items.length - 1 && (
                                                                            <View style={{ width: "77%", alignSelf: "flex-end", marginBottom: 5 }}>
                                                                                <View style={styles.cardFooter}>
                                                                                    <Pressable style={styles.actionButton}
                                                                                    // onPress={() => openGoogleMap(item?.latitude, item?.longitude)}
                                                                                    >
                                                                                        {/* <Image source={Icons.DistanceIcon} resizeMode="contain" style={{ height: heightPixel(21), width: widthPixel(21) }} tintColor={"#264941"} />
                                                                                        <Text style={styles.actionText}>Get Direction</Text> */}
                                                                                    </Pressable>
                                                                                    <View style={styles.statusContainer}>
                                                                                        <Image source={item?.deliveryStatus == "Ready" ? Icons.ReadyPickUpIcon : Icons.PickedUpIcon} resizeMode="contain" style={{ height: heightPixel(14), width: widthPixel(14) }} tintColor={item?.deliveryStatus == "Ready" ? "#FF9D00" : "#109D7D"} />
                                                                                        <Text style={[styles.statusPickedUp, { color: item?.deliveryStatus == "Ready" ? "#FF9D00" : "#109D7D" }]}>{item?.deliveryStatus}</Text>
                                                                                    </View>
                                                                                </View>
                                                                            </View>
                                                                        )}
                                    ...
                                    {/* <View style={styles.cardAmount}>
                                        <View style={styles.leftRow}>
                                            <Text style={styles.paidText}>{orderConfirmationDetails?.paymentStatus}</Text>
                                            <Text style={styles.methodText}>({orderConfirmationDetails?.paymentMode})</Text>
                                        </View>

                                        <Text style={styles.amountText}>{`AED ${orderConfirmationDetails?.totalAmount}`}</Text>
                                    </View> */}
